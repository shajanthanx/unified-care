          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            <div className="flex bg-slate-100 p-1 rounded-lg">
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${userType === 'customer'
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${userType === 'provider'
        ...
        <div className="md:hidden bg-white border-t border-slate-100 animate-in slide-in-from-top-2 duration-200">
          <div className="px-4 pt-2 pb-4 space-y-3">
            ...
            <div className="flex flex-col space-y-2">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Switch View</p>
              <div className="flex bg-slate-100 p-1 rounded-lg">
                <button
                  onClick={() => { setUserType('customer'); setIsMobileMenuOpen(false); }}
                  className={`flex-1 px-3 py-2 rounded-md text-sm font-semibold transition-all ${userType === 'customer'
                      ? 'bg-white text-slate-900 shadow-sm'
                      : 'text-slate-500 hover:text-slate-700'
                    }`}
                >
                ...
                <button
                  onClick={() => { setUserType('provider'); setIsMobileMenuOpen(false); }}
                  className={`flex-1 px-3 py-2 rounded-md text-sm font-semibold transition-all ${userType === 'provider'
                      ? 'bg-white text-indigo-600 shadow-sm'
                      : 'text-slate-500 hover:text-slate-700'
                    }`}
                >
            ...
            <div className="flex items-center space-x-3 py-2">
              <div className="h-10 w-10 rounded-full bg-indigo-100 border-2 border-white shadow-sm overflow-hidden">
                <img
                  src={`https://ui-avatars.com/api/?name=${user.name}&background=random`}
                  alt="User Profile"
                  className="h-full w-full object-cover"
            ...
            <button
              onClick={onSignOut}
              className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center font-medium rounded-lg"
            >
              <LogOut size={16} className="mr-2" /> Sign out
            </button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] rounded-2xl px-5 py-3.5 text-sm shadow-sm ${msg.role === 'user'
        ...
        <div className="divide-y divide-slate-100">
          {filteredTickets.map(ticket => (
            <div key={ticket.id} className="grid grid-cols-12 gap-4 p-5 items-center hover:bg-slate-50 transition-colors group">
              ...
              <div className="col-span-2">
                <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold ${ticket.status === 'Resolved' ? 'bg-green-100 text-green-700' :
